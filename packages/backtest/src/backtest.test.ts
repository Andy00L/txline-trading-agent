  ...
  const odds = (seq: number, tsMs: number, home: number, draw: number, away: number): FeedEvent => ({
    ...
    envelope: {
      ...
      payload: {
        ...
        period: 'full-game',
        lines: [
          { outcome: 'home', label: 'home', decimalOddsMilli: oddsMilli(home), impliedPct: null },
          { outcome: 'draw', label: 'draw', decimalOddsMilli: oddsMilli(draw), impliedPct: null },
          { outcome: 'away', label: 'away', decimalOddsMilli: oddsMilli(away), impliedPct: null },
        ],
      },
    },
  });
